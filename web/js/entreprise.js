application.define("entreprise", function () {
   ...
   */
  $("#companies-table").find("tbody").on("click", ".row-actions button",
      function () {
  ...
  function registerContactPrivate() {
    var form = application.require("utils/forms").getForm(
    ...
    if (values["email-contact"] != "" && contacts.findByEmail(
      ...
      application.require("utils/forms").tagInput(
          form.$.find("input[name='email-contact']"), "danger",
          errors.translate(466)
      );

      return true;
    }

    contacts.create(values, function (resp) {
      ...
      form.clean();
      setTimeout(function () {
        $("#popup-form-registration input[name='companyPk']").val(
            resp.company.pk)
      }, 50);
    }, application.require("utils/ajax").errorHandler(function (xhr) {
      if (xhr.responseJSON == 466) {
        form.tagInput(
            form.$.find("input[name='email-contact']"), "danger",
            errors.translate(466)
        );
  ...
  function createEntreprise() {
    ...
    if (companies.findByName(values.name).length != 0) {
      ...
      console.log("in");
      application.require("utils/ajax").errorHandler(undefined,
          $("#EntPopupFormAlert"), errors.translate(473))();
      return;
    }
          ...
          notifications.notify("Ajout de l'entreprise réussie", "info", "user");
            ...
            function (xhr) {
              if (xhr.responseJSON == 473) {
                console.log(errors.translate(xhr.responseJSON));
                application.require("utils/forms").tagInput(
                    $("#popup-name-entreprise"), "warning",
        ...
        application.require("utils/ajax").errorHandler(
            function (xhr) {
              if (xhr.responseJSON == 473) {
                ...
                companies.invalidate(values.pk).done(function () {
                  application.require("entreprise", "showEdit", {
                    pk: values.pk
                  });
                  alerteForm("EntPopup",
                      "Cette société a été modifié entre temps, vos "
            ...
            "Une erreur est survenue au cours de la mise à jour de l'entreprise")
    );
  }
  ...
  function displayEntreprise(pk) {
    companies.load(pk).done(function () {
      ...
      application.require("utils/popups").load("company", data,
          function (modal) {
            modal.setSize("lg");
            ...
            modal.$.find(".popup-confirm:first").attr("data-view",
                "entreprise:showEdit");
            modal.$.find(".popup-confirm:first").attr("data-context",
                    ...
                    for (var k in data) {
                      if (data[k].je != null) {
                        $e.append(
                            ...
                            + showEtatParticipation(data[k].state)
                            + "</span></td>"
                            + "<td>" + ((data[k].je.payed) ? "Oui" : "Non")
                            + "</td>"
                            + "</tr>"
                        );
                      } else {
