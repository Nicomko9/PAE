...
public class ServletUtil {
  ...
  private static Genson genson = new GensonBuilder().useIndentation(true).useRuntimeType(true)
      .useDateFormat(new SimpleDateFormat("dd/MM/yyyy")).useConstructorWithArguments(true)
      .include(List.class).create();
  ...
  public ContactDto fillContactDto(Map<String, Object> requestMap, ContactDto contactDto,
    ...
    if (!requestMap.get("companyPk").equals("")) {
      Logger.log("Servletutil", "fillcontactdto", "company", Logger.INFORMATION);
      comp = (Company) compUcc
          .getCompanyByPk(Integer.parseInt((String) requestMap.get("companyPk")));
    } else {
      Logger.log("Servletutil", "fillcontactdto", "entreprise", Logger.INFORMATION);
    ...
    if (!requestMap.get("tel-contact").equals("")) {
      ...
      Logger.log("ServletUtil", "fillContactDto", "Empty Phone", Logger.INFORMATION);
      contactDto.setPhone("");
    }
    if (!requestMap.get("email-contact").equals("")) {
      contactDto.setEmail((String) requestMap.get("email-contact"));
    } else {
      Logger.log("ServletUtil", "fillContactDto", "Empty Email", Logger.INFORMATION);
      contactDto.setEmail("");
    }

    if (requestMap.containsKey("active")) {
      contactDto.setActive((Boolean) requestMap.get("active"));
    }
  ...
  public ParticipationDto fillParticipationDto(ParticipationDto participationDto,
    ...
    Logger.log("ServletUtil", "fillParticipationDto", "Fill", Logger.INFORMATION);
