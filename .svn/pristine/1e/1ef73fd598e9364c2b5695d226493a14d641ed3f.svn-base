...
public class DataBaseMock {
...

  {
    ...
    users.add(user);
    user = BusinessFactory.getUser();
    fillUser(user, "clement.dujardin@student.vinci.be", "Clément", "Du Jardin", "cdj", "motdepasse",
        users.size() + 1);
    user.setResponsible(true);
    users.add(user);
    user = BusinessFactory.getUser();
    ...
    je.setDate(LocalDate.of(2015, 10, 25));
    jes.add(je);
    je = BusinessFactory.getJe();
    je.setDate(LocalDate.of(2016, 10, 25));
    jes.add(je);
//    je = BusinessFactory.getJe();
//    je.setDate(LocalDate.of(2017, 10, 25));
//    jes.add(je);
...
//    
  ...
  public List<PresenceDto> getPresencesForContact(int pk) {
    return presences.stream().filter(p -> p.getContact().getPk() == pk)
        .collect(Collectors.toList());
  }
