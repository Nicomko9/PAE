...
public class JeUccImpl implements JeUcc {
  ...
  JeUccImpl(DalFrontEndServices dal, JeDao jeDao, UnitOfWork uof) {
    this.dal = dal;
    this.jeDao = jeDao;
    this.uof = uof;
  }
  ...
  public JeDto createNewJe(JeDto jeDto) {
    ...
    try {
      ...
      jeDto = (JeDto) this.uof.getInsertedObject();
      
      return jeDto;
    } catch (Exception exception) {
      Logger.log("JeUccImpl", "CreateJe", "Creating je : " + exception.getMessage(), Logger.ERROR);
