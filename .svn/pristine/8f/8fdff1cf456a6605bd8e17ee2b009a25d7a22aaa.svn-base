...
public class AppConfig {
  ...
  private Map<Class<?>, Object> impl = new HashMap<>();
  ...
  public AppConfig(String environment) {
    ...
    this.impl.put(AppConfig.class, this);
  }
  ...
  public <T> T newInstanceOf(Class<T> className) throws Exception {
    try {
      ...
      if (this.impl.containsKey(impl)) {
        try {
          return (T) this.impl.get(impl);
        ...
        try {
          ...
          Logger.log("AppConfig", "newInstanceOf",
              "Skipping constructor - error happened : " + exc.getMessage() + " "
                  ...
                  + exc.getStackTrace()[0].getLineNumber(), Logger.ERROR);
        }
      }
      ...
      Logger.log("appConfig", "newInstanceOf", exception.getMessage(), Logger.ERROR);
  ...
  public boolean load() {
    ...
    try (FileInputStream file = new FileInputStream(configurationFile)) {
      ...
      Logger.log("AppConfig", "load", "Internel system error : exiting application", Logger.ERROR);
