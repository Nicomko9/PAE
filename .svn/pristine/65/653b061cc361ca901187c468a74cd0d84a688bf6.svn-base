...
class ContactDaoImpl extends AbstractDao implements ContactDao, ContactSchema {
  ...
  public ContactDto findByPk(int pk) {
    if (this.cacheContains(pk)) {
      return (ContactDto) this.cacheGet(pk);
    }

    return (ContactDto) this.cacheStore(pk, (BizObject) findBy(FIELD_CONTACT_PK, "" + pk));
  }
  ...
  private ContactDto findBy(String key, String value) {
    ...
    try {
      ...
      Logger.log("ContactDaoImpl", "findBy", "The contact was found", Logger.SUCCESS);
  ...
  public Set<ContactDto> search(String... criteria) {
    ...
    try {
      ...
      for (String data : criteria) {
        ...
        buf.append("(LOWER(" + FIELD_CONTACT_FIRSTNAME + ") LIKE ? OR "
            + "LOWER(" + FIELD_CONTACT_LASTNAME + ") LIKE ? ) AND ");
